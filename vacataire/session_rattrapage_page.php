...
// *** CHANGE 1: Requête SQL pour la session de rattrapage ***
...
<body>
    ...
    <div class="page-flex">
        ...
        <div class="main-wrapper">
            ...
            <div class="page-container">
                <div class="content-wrapper">
                    <div class="header-section">
                        <!-- *** CHANGE 3: Titre et sous-titre *** -->
                        <h1 class="main-title">Gestion des Notes</h1>
                        <p class="subtitle">Session Rattrapage - Importation et gestion des fichiers PDF</p>
                    </div>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-value">
                                <?php 
                                // *** CHANGE 4: Vérifier la bonne colonne ***
                                $files_count = 0;
                                $result->data_seek(0);
                                while ($row = $result->fetch_assoc()) {
                                    if (!empty($row['fichier_notes_rattrapage'])) $files_count++;
                                }
                                echo $files_count;
                                $result->data_seek(0);
                                ?>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h2 class="card-title"><i class='bx bx-folder-open'></i> Modules Assignés</h2>
                            <p class="card-subtitle">Gérez vos fichiers PDF de notes pour chaque module de la session de rattrapage</p>
                        </div>
                                    ...
                                    <?php if ($result->num_rows > 0): ?>
                                        <?php while ($row = $result->fetch_assoc()): ?>
                                            <tr id="module-row-<?= $row['id'] ?>">
                                                <td><span class="badge badge-primary"><?= htmlspecialchars($row['code_module']) ?></span></td>
                                                <td><div class="module-title"><?= htmlspecialchars($row['intitule_module']) ?></div></td>
                                                <td><span class="badge badge-success">Semestre <?= htmlspecialchars($row['semestre']) ?></span></td>
                                                <td><strong><?= htmlspecialchars($row['filiere']) ?></strong></td>
                                                <td>
                                                    <form onsubmit="return false;" class="file-upload">
                                                        <!-- *** CHANGE 5: Envoyer 'rattrapage' comme type de session *** -->
                                                        ...
                                                        <label for="file-<?= $row['id'] ?>" class="file-upload-label">
                                                            <i class='bx bx-cloud-upload'></i> Sélectionner PDF
                                                        </label>
                                                    </form>
                                                </td>
                                                <td id="actions-cell-<?= $row['id'] ?>">
                                                    <div class="actions-cell">
                                                        <!-- *** CHANGE 6: Vérifier la bonne colonne *** -->
                                                        <?php if (!empty($row['fichier_notes_rattrapage'])): ?>
                                                            <a href="<?= htmlspecialchars($row['fichier_notes_rattrapage']) ?>" target="_blank" class="btn btn-success"><i class='bx bx-show'></i> Visualiser</a>
                                                            <button class="btn btn-danger" onclick="deleteNotes(<?= $row['id'] ?>, 'rattrapage')"><i class='bx bx-trash'></i> Supprimer</button>
                                                        <?php else: ?>
                                                            <span class="no-file-text"><i class='bx bx-info-circle'></i> Aucun fichier uploadé</span>
                                                        <?php endif; ?>
                                                    </div>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="6"><div class="empty-state"><i class='bx bx-folder-open'></i><h3>Aucun module assigné</h3><p>Vous n'avez aucun module affecté pour cette session.</p></div></td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
            ...
            <div id="toast-notification" class="toast"><i class='bx bx-check-circle'></i><div class="toast-message" id="toast-message"></div></div>
        </div>
    </div>
    
    <!-- Le JavaScript est identique à la page normale, il est conçu pour être réutilisable -->
    <script>
    // Copier/coller l'intégralité du bloc <script> de normale_note.php ici
